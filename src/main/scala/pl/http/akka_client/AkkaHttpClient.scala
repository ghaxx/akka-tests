...
//  implicit val executionContext = ExecutionContext.fromExecutor(Executors.newWorkStealingPool(8))
  ...
  for {
    a <- Http().singleRequest(HttpRequest(uri = "http://localhost:8080/random"))
    b <- Http().singleRequest(HttpRequest(uri = "http://localhost:8080/random"))
    c <- Http().singleRequest(HttpRequest(uri = "http://localhost:8080/random"))
  } yield{
